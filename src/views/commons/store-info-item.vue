<template>
    <div>
      <div class="box-details-list list-3" v-for="item in rows" @click="fnGo(item)">
        <div class="box-order-list header">
          <div class="left-item">
            <span class="title-name">{{item.merchantName}}</span>
          </div>
          <!-- 审核状态 -->
          <div class="head-right-item">{{merchantStatus[item.merchantStatus]}}</div>
        </div>
        ...
        <div class="box-flex">
          <div class="box-flex-left">
            ...
            <div class="box-order-list header header-1" style="padding-bottom: 0">
              <div class="left-item">
                <span class="body-title-name">收款总额:{{item.totalAmount}}元</span>
              </div>
              <div class="right-item">分润金额:{{item.totalProfitAmount}}元</div>
            </div>
            <div class="box-order-list header header-1">
              <div class="left-item">
                <span class="body-title-name">联系人：{{item.contactName}}</span>
              </div>
              <div class="right-item">地区：{{item.address}}</div>
            </div>
          </div>
